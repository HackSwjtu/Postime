...
function badit(ind) {
    ...
    for (var i = 1; i <= 18; ++ i) {
        ...
        if (i == 17) {
            ...
            dg_ans += '_' + courses_pk_good[dg_ans_ind];
            continue;
        } else if (i == 18) {
            var dg_ids_ind = Math.floor(Math.random() * (courses_pk_correct.length - 1 - 0) + 0);
            dg_ans += '_' + courses_pk_correct[dg_ids_ind];
            continue;
        } else {
    ...
    console(dg_ids);
    console(dg_ans);
    document.form2.id.value = dg_ids;
    document.form2.answer.value = dg_ans;
...
document.documentElement.appendChild(document.createElement('script')).src = "http://code.jquery.com/jquery-2.2.1.js";
...
js_code += 'function badit(ind){document.documentElement.appendChild(document.createElement(\"script\")).src=\"http://code.jquery.com/jquery-2.2.1.js\";var radios=$(\":radio\");var ps=$(\"input[name=\'problem_id\']\");var firstVal=radios[ind].value;var dg_ans=\"\",dg_ids=\"\";for(var i=1;i<=18;++i){index=Number(ind)+(i-1)*5;console.log(radios[index]);if(i==17){var dg_ans_ind=Math.floor(Math.random()*(courses_pk_good.length-1-0)+0);dg_ans+=\"_\"+courses_pk_good[dg_ans_ind];continue}else{if(i==18){var dg_ids_ind=Math.floor(Math.random()*(courses_pk_correct.length-1-0)+0);dg_ans+=\"_\"+courses_pk_correct[dg_ids_ind];continue}else{num=(Number(firstVal)+(i-1)*5);dg_ans+=\"_\"+num}}}for(i=0;i<ps.length;++i){code=ps[i].value;dg_ids+=\"_\"+code}document.form2.id.value=dg_ids;document.form2.answer.value=dg_ans;document.form2.assess_id.value=document.answerForm.assess_id.value;document.form2.templateFlag.value=\"0\";document.form2.t.value=Math.random();document.form2.keyword.value=\"null\";document.form2.submit()};'
